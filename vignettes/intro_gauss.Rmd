...
Adding an index for the $g$-th iteration gives us the sensitivity of the posterior sample $\beta^{i}$ at iteration $g$ w.r.t. $B_0^{j}$:
$$\dfrac{d \beta^{i}_{(g)}}{dB_0^{j}} \quad i = 1, 2, 3, 4,\;\; j = 1, 2, ..., 16, \;\; g = 1, 2, ..., `r nrow(d_beta_on_d_B0)`$$.

### Posterior sensitivity
As a measure of prior robustness, we can examine the Jacabian, e.g. $E\left(\dfrac{d\beta}{d B_0}\right)$,
```{r}
jacobian(d_beta_on_d_B0, dim0 = c(4, 16))
```

We can also examine how sensitive the posterior samples of $\beta^1_{(g)}$ are to a change in the, say $(1,2)$, entry of the covariance matrix $B_0$ at each iteration (which is the 5-th element on the matrix as $B_0$ has dimension $4\times 4$). 

```{r}
sens <- d_beta_on_d_B0[, 1, 5]
plot(sens, 
     main = "Sensitivity of posterior samples w.r.t the B_0[1,2]",
     xlab = "Iteration", ylab = "Sensitivity")
```


### Convergence analysis
Then, we can plot out the sensitivity of the sample posterior mean against the number of iterations, i.e. $d\left(\displaystyle\dfrac 1 n \sum_{g=1}^n \beta^1_{(g)}\right) \bigg /dB_0^5$ against $n$, where again $\beta^1_{(g)}$ is the posterior sample of $\beta^1$ at iteration $g$. 
```{r}
csens <- cumulative_stat(sens, mean)
plot(csens, type = "l", 
