...
### Diagnostic 
MCMC theory suggests the sensitivity of the posterior draws to the initial value goes to zero as the draws approach stationarity. 

```{r}
init_val_sens <- get_sensitivity(res, "d_beta", "d_sigma2_0")
plot0(log(apply(init_val_sens, 1, frobenius_norm)), 
      log(apply(init_val_sens, 1, maximum_norm)))
```

Indeed, given that our model specification aligns with the data generating process, we observe extremely fast decay of sensitivities. 
...
# pd0 <- pdmatrix(2)
# data1 <- SUR2_data(
#   n, p, k, Sigma = pd0$Sigma,
#   intercept_1 = TRUE, intercept_2 = TRUE
# )
# str(data1)

# data1 <- student_t_data(n = 100, p = 3, beta = c(0.03, 0.2, -0.3, 0.4), df = 2, intercept = T)

library(glmSimData)
X <- generate_dependent_covariates(n, p)$covariates
y <- generate_response(X, beta = c(0.2, -0.3, 0.4), family = poisson())
res <- gaussian_AD(
  X = as.matrix(X), y = log(1 + y$resp_var) + rnorm(n),
  b_0 = numeric(p), B_0 = diag(p),   # add one for intercept
  alpha_0 = 13, delta_0 = 8,
  num_steps = 2000
)
